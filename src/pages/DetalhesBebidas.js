...
class DetalhesBebidas extends React.Component {
  ...
  recomendedRecipes() {
      ...
      recomendedMeals.push(
        ...
        >
          <img
            ...
            width="180"
          />
          <p>{recomended.meals[index].strCategory}</p>
    ...
    return <div className="recomended">{recomendedMeals}</div>;
  }
  ...
  drinkDetails() {
    ...
    ingredientArray.push(
      Object.entries(drink).filter((ingredient) => ingredient[0].includes('Ingredient')),
    );
    measureArray.push(
      Object.entries(drink).filter((ingredient) => ingredient[0].includes('Measure')),
    );
  ...
  render() {
    ...
    return <div>{loading ? <Loading /> : this.drinkDetails()}</div>;
  }
}
